// Palette data + the one pure function that applies a palette to a tool's
// state. Kept free of any DOM or UI import so both the browser app and the
// headless api.js can use it.
import { toHex, toolState } from './core.js';
...
// Apply `palette` to a tool's colour slots inside `store` (defaults to the
// shared toolState). Cycles the palette so EVERY slot is filled: tools declare
// 1-6 colour slots while palettes hold 5, and a one-to-one mapping used to
// leave a 6-slot tool's last colour stuck on its previous value.
export function applyPaletteToTool(tool, palette, store) {
  if (!tool || !palette || !palette.length) return false;
  const target = store || toolState;
  const colorCtrls = tool.controls.filter(c => c.type === 'color');
  if (!target[tool.slug]) target[tool.slug] = {};
  const s = target[tool.slug];
  let changed = false;
